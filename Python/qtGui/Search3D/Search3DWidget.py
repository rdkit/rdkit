...
class Search3DWidget(_Form,PharmacophoreMixin,ExcludedVolumeMixin):
  ...
  def cdk2Init(self,*args):
    ...
    self.molFilename.setText('test_data/1oir-xtal.mol')
    self.proteinFilename.setText('test_data/1OIR-nowater.pdb')
    self.loadMol()
    self.updateProtein()
    self.tabWidget.setCurrentPage(1)
    #self.viewer3d.server.do('zoom %s'%LocalConfig.pymolTemplateName)
    self.fdefFilename.setText('test_data/SimpleFeatures.fdef')
    #self.fdefFilename.setText('test_data/BaseFeatures.fdef')
    self.loadFDefs()

    self.sdFilename.setText('test_data/cdk2-syn-clip100.sdf')
    self.loadMolsButton.setEnabled(True)
    self.addMolsButton.setEnabled(True)
  ...
  def sarsInit(self,*args):
    ...
    self.molFilename.setText('test_data/1P9U_ligand.mol')
    self.proteinFilename.setText('./test_data/1P9U-ChainB.pdb')
    self.loadMol()
    self.updateProtein()
    self.viewer3d.server.do('zoom %s'%LocalUtils.pymolTemplateName)
    self.fdefFilename.setText('test_data/SimpleFeaturesfdef')
    self.fdefFilename.setText('test_data/BaseFeatures.fdef')
    self.loadFDefs()
