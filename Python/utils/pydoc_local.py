...
# MODIFIED By Greg Landrum of Rational Discovery LLC
#  Original version: 1.56.8.9 (distributed with python 2.2.3)
#
...
# RD-RD-RD
def allfunctions(mod,funcs,fdict,includeAllRoutines=0):
    for key,value in inspect.getmembers(mod,inspect.isroutine):
        if includeAllRoutines or inspect.isbuiltin(value) or inspect.getmodule(value):
...
<tr><td bgcolor="%s">%s</td><td>%s</td>''' % (bgcol, marginalia, gap)
    ...
    def bigsection(self, title, *args):
        ...
        title = '<a name=Sect%s><big><strong>%s</strong></big></a>' % (title,title)
    ...
    def docmodule(self, object, name=None, mod=None, *ignored):
        ...
        funcs, fdict = [], {}
        # RD-RD-RD
        allfunctions(object,funcs,fdict,_includeAllRoutines)
        data = []
        ...
        linksTo = []
        if modules:
            linksTo.append('Modules')
        if classes:
            linksTo.append('Classes')
        if funcs:
            linksTo.append('Functions')
        if data:
            linksTo.append('Data')
        if linksTo:
            linksTo='&nbsp;&nbsp;'.join(['<a href="#Sect%s">%s</a>'%(x,x) for x in linksTo])
            result = result + """<font face="helvetica, arial">
            <p>Jump To: %s</p>
            </font>\n"""%(linksTo)
...
def cli():
    ...
    try:
        # RD-RD-RD (added -a option)
        opts, args = getopt.getopt(sys.argv[1:], 'gk:p:wa')
        writing = 0
...
%s -a
    Include documentation for all routines in modules (can be excessively
    verbose, but this is required to see module-level functions in BPL
    extension modules)
