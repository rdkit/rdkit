...
class SubshapeBuilder(object):
  ...
  def GenerateSubshapeSkeleton(self,conf,shape,terminalPtsOnly=False,
                               featFactory=None):
    pts = FindTerminalPtsFromConformer(conf,self.winRad,self.nbrCount)
    pts = ClusterTerminalPts(pts,self.winRad,self.terminalPtRadScale)
    if not terminalPtsOnly:
      pts = AppendSkeletonPoints(shape.grid,pts,self.winRad,self.stepSize)
    for i,pt in enumerate(pts):
      CalculateDirectionsAtPoint(pt,shape.grid,self.winRad)
    if self.featFactory:
      AssignMolFeatsToPoints(pts,conf.GetOwningMol(),self.featFactory,self.winRad)
    shape.skelPts=pts
