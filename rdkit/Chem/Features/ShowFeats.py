...
def ShowMolFeats(mol, factory, viewer, radius=0.5, confId=-1, showOnly=True, name='',
  ...
  for feat in molFeats: # `for feat in molFeats` would be better than `for i, feat in enumerate(molFeats)`
    family = feat.GetFamily()
    if family in excludeTypes:
      continue
    pos = feat.GetPos(confId)
    color = colors.get(family, (.5, .5, .5))
    # nm = '%s(%d)' % (family, i + 1)  Unused variables

    transparentLevel = 1 - transparency if transparency else 1
    _globalSphereCGO.extend([ALPHA, transparentLevel])
...
if __name__ == '__main__':
  ...
  if options.featMapFile:
    options.featMapFile = sys.stdout if options.featMapFile == '-' else file(options.featMapFile, 'w+')
  ...
  for midx, molN in enumerate(args):
    featLabel = '%s_Feats' % molN if molN != '-' else 'Mol%d_Feats' % (midx + 1)
