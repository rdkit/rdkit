...
def _getAtomInvariantsWithRadius(mol, radius):
  ...
  for i in range(mol.GetNumAtoms()):
    info = {}
    rdMolDescriptors.GetMorganFingerprint(mol, radius, fromAtoms=[i], bitInfo=info)
...
def _calculateBeta(mol, distmat, aid1):
  ...
  for b in bonds:
    ...
    dmax = max(dmax, max([distmat[aid1][bid1], distmat[aid1][bid2]]))
  dmax2 = dmax / 2.0
  return -math.log(0.1) / (dmax2 * dmax2)
...
def CalculateTorsionWeights(mol, aid1=-1, aid2=-1, ignoreColinearBonds=True):
  ...
  if aid1 < 0 and aid2 < 0:
    ...
    if mol.GetBondBetweenAtoms(aid1, aid2) is None:
  ...
  weights = []
  for bid1, bid2, _, _ in bonds:
    if (bid1, bid2) == (aid1, aid2) or (bid2, bid1) == (aid1, aid2):  # if it's the most central bond itself
      d = 0
    else:
      # get shortest distance between the 4 atoms and add 1 to get bond distance
      d = min(distmat[aid1][bid1], distmat[aid1][bid2], 
              distmat[aid2][bid1], distmat[aid2][bid2]) + 1

    weights.append(math.exp(-beta * (d * d)))
...
def CalculateTFD(torsions1, torsions2, weights=None):
  ...
  for tors1, tors2 in zip(torsions1, torsions2):
    ...
    for t1 in tors1[0]:
      for t2 in tors2[0]:
        diff = abs(t1 - t2)
        if diff > 180.0:  # we do not care about direction
          diff = 360.0 - diff
        #print t1, t2, diff
...
def GetTFDBetweenConformers(mol, confIds1, confIds2, useWeights=True, maxDev='equal', symmRadius=2,
  ...
  if useWeights: 
    ...
    return [CalculateTFD(t1, t2, weights=weights) for t1 in torsions1 for t2 in torsions2]
  return [CalculateTFD(t1, t2) for t1 in torsions1 for t2 in torsions2]
...
def GetTFDBetweenMolecules(mol1, mol2, confId1=-1, confId2=-1, useWeights=True, maxDev='equal',
  ...
  if useWeights:
    ...
    return CalculateTFD(torsion1, torsion2, weights=weights)
  return CalculateTFD(torsion1, torsion2)
...
def GetTFDMatrix(mol, useWeights=True, maxDev='equal', symmRadius=2, ignoreColinearBonds=True):
  ...
  if useWeights:
    weights = CalculateTorsionWeights(mol, ignoreColinearBonds=ignoreColinearBonds)
    return [CalculateTFD(torsions[i], torsions[j], weights=weights) for i in range(0, numconf) for j in range(0, i)]
  return [CalculateTFD(torsions[i], torsions[j]) for i in range(0, numconf) for j in range(0, i)]
