...
def GetTFDBetweenConformers(mol, confIds1, confIds2, useWeights=True, maxDev='equal', symmRadius=2,
  ...
  weights = CalculateTorsionWeights(mol, ignoreColinearBonds=ignoreColinearBonds) if useWeights else None
  # weights must be None by default
  tfd = [CalculateTFD(t1, t2, weights=weights) for t1 in torsions1 for t2 in torsions2]
  return tfd
...
def GetTFDBetweenMolecules(mol1, mol2, confId1=-1, confId2=-1, useWeights=True, maxDev='equal',
  ...
  weights = CalculateTorsionWeights(mol1, ignoreColinearBonds=ignoreColinearBonds) if useWeights else None
  # weights must be None by default
...
def GetTFDMatrix(mol, useWeights=True, maxDev='equal', symmRadius=2, ignoreColinearBonds=True):
  ...
  weights = CalculateTorsionWeights(mol, ignoreColinearBonds=ignoreColinearBonds) if useWeights else None
  # weights must be None by default
  tfdmat = [CalculateTFD(torsions[i], torsions[j], weights=weights) for i in range(0, numconf) for j in range(0, i)]
  return tfdmat
