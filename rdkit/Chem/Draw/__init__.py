...
def MolToImage(mol, size=(300, 300), kekulize=True, wedgeBonds=True,
    ...
    if legend:
        ...
        # the 0.94 is extremely empirical
        pos = size[0] / 2, int(.94 * size[1]), 0
...
def ShowMol(mol, size=(300, 300), kekulize=True, wedgeBonds=True,
    ...
    """
    MolToImage(mol, size, kekulize, wedgeBonds, **kwargs).show(title)
...
def calcAtomGaussians(mol, a=0.03, step=0.02, weights=None):
  ...
  fig.savefig('coumlogps.colored.png',bbox_inches='tight')
    ...
    Z = mlab.bivariate_normal(
        X, Y, a, a, mol._atomPs[0][0], mol._atomPs[0][1]) * weights[0]
    for i in range(1, mol.GetNumAtoms()):
        Zp = mlab.bivariate_normal(
            X, Y, a, a, mol._atomPs[i][0], mol._atomPs[i][1])
        Z += Zp * weights[i]
    return X, Y, Z
...
def MolsToImage(mols, subImgSize=(200, 200), legends=None, **kwargs):
    ...
    for i, mol in enumerate(mols):
        res.paste(MolToImage(
            mol, subImgSize, legend=legends[i], **kwargs), (i * subImgSize[0], 0))
    return res
...
def MolsToGridImage(mols, molsPerRow=3, subImgSize=(200, 200), legends=None,
    ...
    res = Image.new(
        "RGBA", (molsPerRow * subImgSize[0], nRows * subImgSize[1]), (255, 255, 255, 0))
    for i, mol in enumerate(mols):
        row = i // molsPerRow
        col = i % molsPerRow
        highlights = None
