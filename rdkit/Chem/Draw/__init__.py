...
def DrawMorganEnv(mol,atomId,radius,molSize=(150,150),baseRad=0.3,svg=True,
                  aromaticColor=(0.9,0.9,0.2),ringColor=(0.8,0.8,0.8),centerColor=(0.6,0.6,0.9),
                  extraColor=(.9,.9,.9),
                  **kwargs):
    ...
    drawer.DrawMolecule(submol,highlightAtoms=highlightAtoms,highlightAtomColors=atomcolors,
                    highlightBonds=highlightBonds,highlightBondColors=bondcolors,highlightAtomRadii=highlightRadii,
                    **kwargs)
    drawer.FinishDrawing()
    return drawer.GetDrawingText()

def DrawRDKitBit(mol,bitId,bitInfo,**kwargs):
    bondPath = bitInfo[bitId][0]
    return DrawRDKitEnv(mol,bondPath,**kwargs)

def DrawRDKitEnv(mol,bondPath,molSize=(150,150),baseRad=0.3,svg=True,
                  aromaticColor=(0.9,0.9,0.2),nonAromaticColor=None,
                  extraColor=(.9,.9,.9),
                  **kwargs):
    rdDepictor.Compute2DCoords(mol)
