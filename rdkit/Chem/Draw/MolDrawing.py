...
class MolDrawing(object):
    ...
    def transformPoint(self, pos):
        res = [0, 0]
        res[0] = (pos[0] + self.molTrans[0]) * self.currDotsPerAngstrom * \
            self.drawingOptions.useFraction + self.drawingTrans[0]
    ...
    def _getBondAttachmentCoordinates(self, p1, p2, labelSize):
        newpos = [None, None]
        if labelSize != None:
            labelSizeOffset = [labelSize[0][
                0] / 2 + (cmp(p2[0], p1[0]) * labelSize[0][2]), labelSize[0][1] / 2]
            if p1[1] == p2[1]:
                newpos[0] = p1[0] + cmp(p2[0], p1[0]) * labelSizeOffset[0]
            else:
                if abs(labelSizeOffset[1] * (p2[0] - p1[0]) / (p2[1] - p1[1])) < labelSizeOffset[0]:
                    newpos[0] = p1[
                        0] + cmp(p2[0], p1[0]) * abs(labelSizeOffset[1] * (p2[0] - p1[0]) / (p2[1] - p1[1]))
                else:
                    newpos[0] = p1[0] + cmp(p2[0], p1[0]) * labelSizeOffset[0]
            if p1[0] == p2[0]:
                ...
                if abs(labelSizeOffset[0] * (p1[1] - p2[1]) / (p2[0] - p1[0])) < labelSizeOffset[1]:
                    newpos[1] = p1[
                        1] + cmp(p2[1], p1[1]) * abs(labelSizeOffset[0] * (p1[1] - p2[1]) / (p2[0] - p1[0]))
                else:
                    newpos[1] = p1[1] + cmp(p2[1], p1[1]) * labelSizeOffset[1]
        else:
    ...
    def _drawWedgedBond(self, bond, pos, nbrPos,
        ...
        if not dash:
            ...
            self.canvas.addCanvasDashedWedge(
                poly[0], poly[1], poly[2], color=color)
        else:
    ...
    def _drawBond(self, bond, atom, nbr, pos, nbrPos, conf,
        ...
        newnbrPos = self._getBondAttachmentCoordinates(
            p2_raw, p1_raw, labelSize2)
        ...
        if bType == Chem.BondType.SINGLE:
            ...
            if bDir in (Chem.BondDir.BEGINWEDGE, Chem.BondDir.BEGINDASH):
                ...
                if bDir == Chem.BondDir.BEGINWEDGE:
                    self._drawWedgedBond(
                        bond, p1, p2, color=wcolor, width=width)
                elif bDir == Chem.BondDir.BEGINDASH:
                    self._drawWedgedBond(bond, p1, p2, color=wcolor, width=width,
                                         dash=self.drawingOptions.dash)

            else:
                self.canvas.addCanvasLine(
                    newpos, newnbrPos, linewidth=width, color=color, color2=color2)
            ...
            if not crossBond and \
                ...
                self.canvas.addCanvasLine(
                    newpos, newnbrPos, linewidth=width, color=color, color2=color2)
                fp1, fp2 = self._offsetDblBond(
                    newpos, newnbrPos, bond, atom, nbr, conf)
                self.canvas.addCanvasLine(
                    fp1, fp2, linewidth=width, color=color, color2=color2)
            else:
                ...
                self.canvas.addCanvasLine(
                    fp1, fp2, linewidth=width, color=color, color2=color2)
                self.canvas.addCanvasLine(
                    fp3, fp4, linewidth=width, color=color, color2=color2)
            ...
            self.canvas.addCanvasLine(
                fp1, fp2, linewidth=width, color=color, color2=color2)
            fp1, fp2 = self._offsetDblBond(
                newpos, newnbrPos, bond, atom, nbr, conf, dir=-1)
            self.canvas.addCanvasLine(
                fp1, fp2, linewidth=width, color=color, color2=color2)
        else:
    ...
    def _drawLabel(self, label, pos, baseOffset, font, color=None, **kwargs):
        ...
        y1 = pos[1]
        labelP = x1, y1
        labelSize = self.canvas.addCanvasText(
            label, (x1, y1, baseOffset), font, color, **kwargs)
        return labelSize
    ...
    def AddMol(self, mol, centerIt=True, molTrans=None, drawingTrans=None,
        ...
        font = Font(face=self.drawingOptions.atomLabelFontFace,
                    size=self.currAtomLabelFontSize)
        ...
        includeAtomNumbers = kwargs.get(
            'includeAtomNumbers', self.drawingOptions.includeAtomNumbers)
        self.atomPs[mol] = {}
        ...
        for atom in mol.GetAtoms():
            ...
            if pos is None:
                pos = self.transformPoint(
                    conf.GetAtomPosition(idx) * self.drawingOptions.coordScale)
                self.atomPs[mol][idx] = pos
                if drawAtom:
                    self.boundingBoxes[mol][0] = min(
                        self.boundingBoxes[mol][0], pos[0])
                    self.boundingBoxes[mol][1] = min(
                        self.boundingBoxes[mol][1], pos[1])
                    self.boundingBoxes[mol][2] = max(
                        self.boundingBoxes[mol][2], pos[0])
                    self.boundingBoxes[mol][3] = max(
                        self.boundingBoxes[mol][3], pos[1])
            ...
            for bond in atom.GetBonds():
                ...
                if nbrIdx > idx:
                    ...
                    if nbrPos is None:
                        ...
                        self.boundingBoxes[mol][0] = min(
                            self.boundingBoxes[mol][0], nbrPos[0])
                        self.boundingBoxes[mol][1] = min(
                            self.boundingBoxes[mol][1], nbrPos[1])
                        self.boundingBoxes[mol][2] = max(
                            self.boundingBoxes[mol][2], nbrPos[0])
                        self.boundingBoxes[mol][3] = max(
                            self.boundingBoxes[mol][3], nbrPos[1])
            ...
            if labelIt:
                ...
                if includeAtomNumbers:
                    ...
                    if atom.GetNumRadicalElectrons():
                        rad = self.drawingOptions.radicalSymbol * \
                    ...
                    if atom.HasProp('molAtomMapNumber'):
                        ...
                        mapNumLength = 1 + \
                            len(str(atom.GetProp('molAtomMapNumber')))
                    ...
                    if deg == 0:
                        if periodicTable.GetElementSymbol(atom.GetAtomicNum()) in ('O', 'S', 'Se', 'Te', 'F', 'Cl', 'Br', 'I', 'At'):
                            symbol = '%s%s%s%s%s%s' % (
                                hs, isotope, base, chg, rad, mapNum)
                        else:
                            symbol = '%s%s%s%s%s%s' % (
                                isotope, base, hs, chg, rad, mapNum)
                    elif deg > 1 or nbrSum[0] < 1:
                        symbol = '%s%s%s%s%s%s' % (
                            isotope, base, hs, chg, rad, mapNum)
                        baseOffset = 0.5 - \
                            (isotopeLength + len(base) / 2.) / symbolLength
                    else:
                        symbol = '%s%s%s%s%s%s' % (
                            rad, chg, hs, isotope, base, mapNum)
                        baseOffset = -0.5 + \
                            (mapNumLength + len(base) / 2.) / symbolLength
                    if deg == 1:
                        if abs(nbrSum[1]) > 1:
                ...
                if highlightMap and idx in highlightMap:
                    ...
                    color = self.drawingOptions.elemDict.get(
                        atom.GetAtomicNum(), (0, 0, 0))
                labelSize = self._drawLabel(
                    symbol, pos, baseOffset, font, color=color, orientation=orient)
        ...
        for bond in mol.GetBonds():
            ...
            if highlightBonds and bond.GetIdx() in highlightBonds:
                ...
                if self.drawingOptions.colorBonds:
                    color = self.drawingOptions.elemDict.get(
                        atom.GetAtomicNum(), (0, 0, 0))
                    color2 = self.drawingOptions.elemDict.get(
                        nbr.GetAtomicNum(), (0, 0, 0))
                else:
                    color = self.drawingOptions.defaultColor
                    color2 = color
