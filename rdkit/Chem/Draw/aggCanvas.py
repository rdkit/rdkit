...
faceMap = {'sans': os.path.join(
    RDConfig.RDCodeDir, 'Chem', 'Draw', 'FreeSans.ttf')}
...
class Canvas(CanvasBase):
    ...
    def addCanvasLine(self, p1, p2, color=(0, 0, 0), color2=None, **kwargs):
        if color2 and color2 != color:
            ...
            self._doLine(
                p1, mp, Pen(color, kwargs.get('linewidth', 1)), **kwargs)
            color2 = convertColor(color2)
            self._doLine(
                mp, p2, Pen(color2, kwargs.get('linewidth', 1)), **kwargs)
        else:
            color = convertColor(color)
            self._doLine(
                p1, p2, Pen(color, kwargs.get('linewidth', 1)), **kwargs)

    def addCanvasText(self, text, pos, font, color=(0, 0, 0), **kwargs):
        orientation = kwargs.get('orientation', 'E')
        color = convertColor(color)
        aggFont = Font(
            color, faceMap[font.face], size=font.size * self.fontScale)
        ...
        if not len(blocks):
            ...
            for block in blocks:
                ...
                if fmt in ('sub', 'sup'):
                    lFont = Font(
                        color, faceMap[font.face], size=0.8 * font.size * self.fontScale)
                else:
                    lFont = aggFont
    ...
    def addCanvasDashedWedge(self, p1, p2, p3, dash=(2, 2), color=(0, 0, 0),
        ...
        for i in range(len(pts1)):
            self.draw.line(
                (pts1[i][0], pts1[i][1], pts2[i][0], pts2[i][1]), pen)
