  ...
  def _testSpellings(self, smiList):
    for smi, spellings in smiList:
      ...
      for spelling in spellings:
        ...
        assert canSmi == trySmi, ('Non-canonical: mol %s gave %s ' +
                                  '(should be %s)') % (spelling, trySmi, canSmi)
        m2 = Chem.MolFromSmiles(trySmi.strip())
        try2 = Chem.MolToSmiles(m2)
        assert canSmi == try2, ('Non-canonical: mol %s gave %s (should be %s) ' +
                                'on second pass') % (spelling, try2, canSmi)
