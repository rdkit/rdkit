...
stereo_mols = load_smiles(r"""
C/C=C\N # AKK
CN[C@@H]1[C@@H](O)[C@@H](O)OC[C@]1(C)O # GE3
C[C@H]1[C@H](O)[C@@H](CO)O[C@]1(O)CO # DFR
C[C@@]1(O)[C@H](O)[C@@H](CO)O[C@H]1n1ccc2c(N)ncnc21 # H5V smiles generated by RDKit from PDB inchi
C[C@]1([C@@H]([C@H](O[C@H]1n2ccc3c2ncnc3N)CO)O)O # H5V smiles from RCSB PDB
C1C[C@H]2CC[C@@H]1C2 # I9A
CC(=O)[C@H]1CC[C@@H]2[C@@]1(CC[C@H]3[C@H]2CC[C@@H]4[C@@]3(CC[C@H](C4)OS(=O)(=O)N)C)C # VRI
""")

class TestStereochemistry(MCSTestCase):
  # Matching each molecule to itself should return the same number of atoms and bonds as in molecule

  def test_double_bond_stereo(self):
    # AKK, 4 atoms, 3 bonds
    self.assert_search(SELECT(stereo_mols, 1, 1), 4, 3, matchChiralTag=True)

  def test_single_ring(self):
    # GE3, 12 atoms, 12 bonds
    self.assert_search(SELECT(stereo_mols, 2, 2), 12, 12, matchChiralTag=True)
    # DFR, 12 atoms, 12 bonds
    self.assert_search(SELECT(stereo_mols, 3, 3), 12, 12, matchChiralTag=True)

  def test_ring_system(self):
    # H5V, 20 atoms, 22 bonds
    self.assert_search(SELECT(stereo_mols, 4, 4), 20, 22, matchChiralTag=True)
    self.assert_search(SELECT(stereo_mols, 5, 5), 20, 22, matchChiralTag=True)

  def test_bicyclic_rings(self):
    # I9A, 7 atoms, 8 bonds
    self.assert_search(SELECT(stereo_mols, 6, 6), 7, 8, matchChiralTag=True, ringCompare=rdFMCS.RingCompare.StrictRingFusion)

  def test_fused_rings(self):
    # VRI, 27 atoms, 30 bonds
    self.assert_search(SELECT(stereo_mols, 7, 7), 27, 30, matchChiralTag=True)
    self.assert_search(SELECT(stereo_mols, 7, 7), 27, 30, matchChiralTag=True, ringMatchesRingOnly=True)
    self.assert_search(SELECT(stereo_mols, 7, 7), 27, 30, matchChiralTag=True, ringMatchesRingOnly=True, ringCompare=rdFMCS.RingCompare.StrictRingFusion)
