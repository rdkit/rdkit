...
def FormCounts(bitVects, actVals, whichBit, nPossibleActs, nPossibleBitVals=2):
  ...
  **Notes**
  ...
  res = numpy.zeros((nPossibleBitVals, nPossibleActs), dtype=numpy.int64)
...
def CalcInfoGains(bitVects, actVals, nPossibleActs, nPossibleBitVals=2):
  ...
  **Arguments**
  ...
  res = numpy.zeros(nBits, dtype=numpy.float64)
...
def AnalyzeSparseVects(bitVects, actVals):
  ...
  **Arguments**
  ...
  actives = numpy.zeros(nBits, dtype=numpy.int64)
  inactives = numpy.zeros(nBits, dtype=numpy.int64)
  ...
  resTbl = numpy.zeros((2, 2), dtype=numpy.int64)
  res = []
  gains = []
  for bit in range(nBits):
