...
#include "gmf.h"
int LaGenMatFloat::debug_ = 0;     // turn off global deubg flag initially.
                                // use A.debug(1) to turn on/off,
                                // and A.debug() to check current status.

int* LaGenMatFloat::info_= new int;        // turn off info print flag.

LaGenMatFloat::~LaGenMatFloat()
{}

 LaGenMatFloat::LaGenMatFloat() : v(0)
{
  ...
  LaGenMatFloat::LaGenMatFloat(int m, int n) :v(m*n)
{
    ii[0](0,m-1);
    ii[1](0,n-1);
    dim[0] = m;
  ...
  LaGenMatFloat::LaGenMatFloat(float *d, int m, int n) :v(d, m*n)
{
    if (debug())
    {
		std::cout << ">>> LaGenMatFloat::LaGenMatFloat(float *d, int m, int n) :v(d, m*n)\n";
    }
...
if (debug())
{
	std::cout << ">>> LaGenMatFloat& LaGenMatFloat::operator=(float s)\n";
...
LaGenMatFloat&  LaGenMatFloat::resize(int m, int n)
{
    if (debug())
    {
		std::cout << ">>> resize("<< m << "," << n << ")\n";
    ...
    LaGenMatFloat tmp1(0,0);
    ref(tmp1);

    // now, reference an MxN matrix
    LaGenMatFloat tmp(m,n);
    ref(tmp);
...
if (debug())
{
 ...
 LaGenMatFloat&  LaGenMatFloat::resize(const LaGenMatFloat &A)
{
    if (debug())
    {
		std::cout << ">>> resize(const LaGenMatFloat &A)" << std::endl;
...
LaGenMatFloat::LaGenMatFloat(const LaGenMatFloat& X) : v(0)
{
    if (X.debug())
    {
...
LaGenMatFloat& LaGenMatFloat::copy(const LaGenMatFloat& X) 
{
    if (debug())
    {
	std::cout << ">>> LaGenMatFloat& LaGenMatFloat::copy\
(const LaGenMatFloat& X)\n";
	std::cout << " X: " << X.info() << std::endl;
    }
...
std::ostream& operator<<(std::ostream& s, const LaGenMatFloat& G)
{
