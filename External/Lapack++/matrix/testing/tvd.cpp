...
LaVectorDouble madd(const LaVectorDouble &A, const LaVectorDouble &B)
{
if (A.debug())
{
    std::cout << ">>> madd(A,B) \n";
    ...
    if (A.size() != B.size())
    {
        std::cerr << "LaVectorDouble madd(LaVectorDouble &A, LaVectorDouble &B): \
            non-conformant arrays.\n";

        return LaVectorDouble(0);       // 0x0 matrix
    }

    int i;
    LaVectorDouble result(A.size());
...
int main(int argc, char *argv[])
{
    ...
    LaVectorDouble D(C);        // D is also N,N
