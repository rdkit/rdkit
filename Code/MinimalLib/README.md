...
[RDKit.js](https://rdkitjs.com) is the official JavaScript distribution of cheminformatics functionality from [RDKit](https://github.com/rdkit/rdkit) -
...
The package itself consist of three files:
...
See [rdkitjs.com](https://rdkitjs.com) for example codes, documentation and demos.
...
However, this is not RDKit specific, but generally how you support WASM for those frameworks.
...
Go to [MinimalLib](https://github.com/rdkit/rdkit/tree/master/Code/MinimalLib) directory, and run the following script:

```bash
scripts/build_rdkitjs.sh
```

The script results in `build` directory with a valid javascript module, and includes `RDKit_minimal.{js,wasm,d.ts}`.
To set the version of the javascript module, e.g. `NPM_VERSION=1.2.3`.
Defaults to `0.0.0`.

Set `GET_SRC=clone` to swap the local copy for a fresh clone from GitHub.
Optionally pass a release tag (defaults to `master`) and a git URL (defaults to `https://github.com/rdkit/rdkit.git`):
