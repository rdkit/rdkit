...
SHELL ["/bin/bash", "-c", "-l"]
RUN source /opt/emsdk/emsdk_env.sh && emcmake cmake -DBOOST_ROOT=/opt/boost -DBoost_INCLUDE_DIR=/opt/boost/include -DBoost_USE_STATIC_LIBS=ON -DRDK_BUILD_FREETYPE_SUPPORT=ON -DRDK_BUILD_MINIMAL_LIB=ON \
