...
RUN echo "source /opt/emsdk.git/emsdk_env.sh > /dev/null 2>&1" >> ~/.bashrc \
    && echo "export EM_CACHE=${EM_CACHE}" >> ~/.bashrc
...
RUN emcmake cmake \
    ...
    -DCMAKE_C_FLAGS="${EXCEPTION_HANDLING}" \
    -DCMAKE_EXE_LINKER_FLAGS="${EXCEPTION_HANDLING}" \
...
RUN EM_CACHE=/opt/emscripten-cache \
...
# copy local rdkit source into the image
...
RUN EM_CACHE=/opt/emscripten-cache \
    && BOOST_CMAKE_DIR=$(find ${EM_CACHE}/sysroot/lib/cmake -name BoostConfig.cmake | head -1 | xargs dirname) \
...
# Build
ENV DIST_DIR="../Code/MinimalLib/build/"
RUN make -j2 RDKit_minimal
RUN mkdir -p $DIST_DIR
RUN mkdir -p $DIST_DIR/dist
RUN cp Code/MinimalLib/RDKit_minimal.* $DIST_DIR/dist
