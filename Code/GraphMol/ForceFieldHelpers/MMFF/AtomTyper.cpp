    ...
    switch (atom->getAtomicNum()) {
      ...
      case 7:
        ...
        for (; nbrIdx != endNbrs; ++nbrIdx) {
          ...
          if (((atom->getValence(true) + atom->getNumImplicitHs()) >= 3) &&
        ...
        if (atom->getTotalDegree() == 3) {
          // total bond order >= 4
          if ((atom->getValence(true) + atom->getNumImplicitHs()) >= 4) {
        ...
        if (atom->getTotalDegree() == 2) {
          ...
          // total bond order >= 2
          if ((atom->getValence(true) + atom->getNumImplicitHs()) >= 2) {
            // loop over neighbors
            bool isNSO = false;
            boost::tie(nbrIdx, endNbrs) = mol.getAtomNeighbors(atom);
