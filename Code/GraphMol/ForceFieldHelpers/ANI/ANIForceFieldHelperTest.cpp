...
TEST_CASE("Check ANI Force Field builder") {
  SECTION("ANI-1ccx") {
    ...
    REQUIRE(mol);
    int confId = -1;
    auto field = RDKit::ANI::constructForceField(*mol, "ANI-1ccx");
    field->initialize();
  ...
  SECTION("ANI-1x") {
    ...
    REQUIRE(mol);
    int confId = -1;
    auto field = RDKit::ANI::constructForceField(*mol, "ANI-1x");
    field->initialize();
    auto numAtoms = mol->getNumAtoms();
    double *pos;
    auto conf = mol->getConformer(confId);
  ...
  SECTION("ANI Convenience Test") {
    ...
    auto mol = MolFileToMol(filePath, true, false);
    REQUIRE(mol);
    auto res = RDKit::ANI::ANIOptimizeMolecule(*mol, "ANI-1x");
    CHECK(res.first == 0);
    CHECK(res.second < -40.0517);
    std::vector<std::pair<int, double>> res1;
    RDKit::ANI::ANIOptimizeMoleculeConfs(*mol, res1, "ANI-1x");
  ...
  SECTION("Unsupported Atoms") {
    ...
    auto mol = MolFileToMol(filePath, true, false);
    REQUIRE(mol);
    REQUIRE_THROWS_AS(RDKit::ANI::constructForceField(*mol, "ANI-1x"),
                      ValueErrorException);
  }
}