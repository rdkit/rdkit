...
double getAlignmentTransform(const ROMol &prbMol, const ROMol &refMol,
     ...
     if (atomMap == 0) {
          ...
          if (SubstructMatch(refMol, prbMol, match, recursionPossible,
               ...
               throw MolAlignException(
                   "No sub-structure match found between the probe and query "
                   "mol");
          }
     } else {
          MatchVectType::const_iterator mi;
...
double alignMol(ROMol &prbMol, const ROMol &refMol, int prbCid, int refCid,
     ...
     double res =
         getAlignmentTransform(prbMol, refMol, trans, prbCid, refCid, atomMap,
                               weights, reflect, maxIterations);
...
void alignMolConformers(ROMol &mol, const std::vector<unsigned int> *atomIds,
     ...
     if (confIds == 0) {
          ...
          // Conformer *conf;
          for (cnfi = mol.beginConformers(); cnfi != mol.endConformers();
               cnfi++) {
               // conf = (*cnfi);
               i += 1;
               if (i == 1) {
               ...
               ssd = RDNumeric::Alignments::AlignPoints(
                   refPoints, prbPoints, trans, weights, reflect, maxIters);
               if (RMSlist) {
                    ssd /= (prbPoints.size());
