...
namespace RDDepict {
void CoordinateTemplates::assertValidTemplate(RDKit::ROMol& mol, const std::string& smiles) {
    ...
    if (mol.getConformer().is3D()) {
        std::string msg = "Template has 3D coordinates, 2D coordinates required: " + smiles;
        throw RDDepict::DepictException(msg);
    }

    // Make sure this template is a single ring system (spiro'd ring systems are OK).
    // We can check this by ensuring that every bond is in a ring and that there
    // is only one connected component in the molecular graph.
    if (RDKit::MolOps::getMolFrags(mol).size() != 1) {
        std::string msg = "Template consists of multiple fragments, single fragment required: "
            + smiles;
        throw RDDepict::DepictException(msg);
    }
...
void CoordinateTemplates::loadTemplatesFromPath(const std::string& templatePath,
    ...
    while (std::getline(cxsmiles, line)) {
        ...
        if (!mol_ptr) {
            std::string msg = "Could not load templates from " + templatePath +
                ": Invalid smiles";
            cxsmiles.close();
            throw RDDepict::DepictException(msg);
...
void CoordinateTemplates::setRingSystemTemplates(const std::string& templatePath) {
    // Try loading templates in from this directory, if unsuccessful, keep current templates
    std::unordered_map<unsigned int, std::vector<std::shared_ptr<RDKit::ROMol>>> templates;
...
void CoordinateTemplates::addRingSystemTemplates(const std::string& templatePath) {
    // Try loading templates in from this directory, if unsuccessful, keep current templates
    std::unordered_map<unsigned int, std::vector<std::shared_ptr<RDKit::ROMol>>> templates;
    loadTemplatesFromPath(templatePath, templates);
    for (auto& kv : templates) {
        m_templates[kv.first].insert(m_templates[kv.first].begin(), kv.second.begin(), kv.second.end());
    }
}
} // namespace RDDepict