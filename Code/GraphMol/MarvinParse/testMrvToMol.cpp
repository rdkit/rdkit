    ...
    ChemicalReaction *GetReaction(const RxnTest *rxnTest) {
      ...
      void testMarvinMol(const MolTest *molTest) {
        ...
        try {
...

          {
            ...
            generateNewExpectedFilesIfSoSpecified(fName + ".NEW.sdf",
                                                  outMolStr);
...

          {
            ...
            generateNewExpectedFilesIfSoSpecified(fName + ".NEW.mrv",
                                                  outMolStr);
      ...
      void testMarvinRxn(const RxnTest *rxnTest) {
        ...
        try {
          ...
          TEST_ASSERT(localVars.rxn->getNumAgentTemplates() ==
                      rxnTest->agentCount);
...

          {
            ...
            generateNewExpectedFilesIfSoSpecified(fName + ".NEW.rxn",
                                                  outMolStr);
      ...
      void RunTests() {
        ...
        if (testToRun == "" || testToRun == "molFileTests") {
          std::list<MolTest> molFileTests{
              ...
              MolTest("MultipleSgroupParentInMiddleOfAtomBlock.mrv", true, 23,
                      22),
          ...
          for (auto molFileTest : molFileTests) {
            BOOST_LOG(rdInfoLog)
                << "Test: " << molFileTest.fileName << std::endl;
        ...
        if (testToRun == "" || testToRun == "rxnFileTests") {
          std::list<RxnTest> rxnFileTests{
              ...
              RxnTest("aspirineSynthesisWithAttributes.mrv", true, 2, 0, 1, 3,
                      0),
