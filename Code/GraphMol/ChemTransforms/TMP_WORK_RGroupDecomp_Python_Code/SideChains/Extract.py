  ...
  for molIdx,mol in enumerate(mols):
    ...
    for i,core in enumerate(cores):
      if not core:
        ...
        continue
      coreRes=[]
      tmatches = mol.GetSubstructMatches(core)
      if not tmatches:
        ...
        tMol = Chem.ReplaceCore(mol,core,True,True)
        if tMol:
          molMatched=True
...
def ProcessCoreLabels(cores,options):
  ...
  for coreIdx,core in enumerate(cores):
    ...
    for idx,dummy in enumerate(dummies):
      ...
      if not okSmarts.match(dummy.GetSmarts()): continue
      ...
      nbr = nbrs[0]
      nbr.SetProp('_RLabel',str(label))
      labelDict[nbr.GetIdx()] = label
      rmv.append(dummy.GetIdx())

    em = Chem.EditableMol(core)
