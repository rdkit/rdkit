...
#if 0
int main() {
  std::string filePath = "example.dat";  // Your file path here
  size_t size;
  void *mappedMemory = createMemoryMapping(filePath, size);

  if (mappedMemory != nullptr) {
    std::cout << "File successfully mapped to memory!" << std::endl;

    // Access the memory-mapped file here (e.g., print first byte)
    char *data = static_cast<char *>(mappedMemory);
    std::cout << "First byte: " << data[0] << std::endl;

    unmapMemory(mappedMemory, size);
  }
