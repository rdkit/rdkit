...
class RDKIT_SYNTHONSPACESEARCH_EXPORT SearchShapeInput : public ShapeInput {
 public:
  ...
  // Fills in the transform for the relevant overlay if there is one, plus
  // the conformations of the two that were responsible.  Returns -1.0 for
  // the similarity if there was nothing within the threshold.
  double bestSimilarity(
      SearchShapeInput &fitShape, RDGeom::Transform3D &xform,
      unsigned int &bestThisConf, unsigned int &bestFitConf,
