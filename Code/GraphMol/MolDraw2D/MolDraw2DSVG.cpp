...
void MolDraw2DSVG::drawLine(const Point2D &cds1, const Point2D &cds2) {
  ...
  d_os << "<path ";
  outputClasses();
  d_os << "d='M " << formatDouble(c1.x) << "," << formatDouble(c1.y)
       << " L " << formatDouble(c2.x) << "," << formatDouble(c2.y)
       << "' ";
  d_os << "style='fill:none;fill-rule:evenodd;stroke:" << col
       << ";stroke-width:" << formatDouble(width)
       << "px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"
       << dashString << "'";
  d_os << " />\n";
}
...
void MolDraw2DSVG::drawPolygon(const std::vector<Point2D> &cds) {
  ...
  d_os << " " << formatDouble(c0.x) << "," << formatDouble(c0.y);
  for (unsigned int i = 1; i < cds.size(); ++i) {
    Point2D ci = getDrawCoords(cds[i]);
    d_os << " L " << formatDouble(ci.x) << "," << formatDouble(ci.y);
  }
  if (fillPolys()) {
  ...
  d_os << "stroke:" << col << ";stroke-width:" << formatDouble(width)
...
void MolDraw2DSVG::clearDrawing() {
  ...
  d_os << " width='" << formatDouble(width())
       << "' height='" << formatDouble(height()) << "'";
  d_os << " x='" << formatDouble(offset().x)
...
void MolDraw2DSVG::addMoleculeMetadata(const ROMol &mol, int confId) const {
  ...
  for (const auto atom : mol.atoms()) {
    ...
    d_os << " drawing-x=\"" << formatDouble(dpos.x) << "\""
         << " drawing-y=\"" << formatDouble(dpos.y) << "\"";
    d_os << " x=\"" << formatDouble(pos.x) << "\""
         << " y=\"" << formatDouble(pos.y) << "\""
         << " z=\"" << formatDouble(pos.z) << "\"";
...
void MolDraw2DSVG::tagAtoms(const ROMol &mol, double radius,
  ...
  for (const auto &bond : mol.bonds()) {
    ...
    if (drawOptions().splitBonds) {
      ...
      // from begin to mid
      d_os << "<path "
           << " d='M " << formatDouble(a1pos.x)
           << "," << formatDouble(a1pos.y)
           << " L " << formatDouble(midp.x) << ","
           << formatDouble(midp.y) << "'";
      ...
      // mid to end
      d_os << "<path "
           << " d='M " << formatDouble(midp.x) << "," << formatDouble(midp.y)
           << " L " << formatDouble(a2pos.x) << "," << formatDouble(a2pos.y)
           << "'";
      d_os << " class='bond-selector bond-" << this_idx << " atom-" << a_idx2;
      outputTagClasses(bond, d_os, d_activeClass);
      d_os << "'";
      d_os << " style='fill:#fff;stroke:#fff;stroke-width:"
           << formatDouble(width) << "px;fill-opacity:0;"
              "stroke-opacity:0' ";
      d_os << "/>\n";
    } else {
      d_os << "<path "
           << " d='M " << formatDouble(a1pos.x) << "," << formatDouble(a1pos.y)
           << " L " << formatDouble(a2pos.x) << "," << formatDouble(a2pos.y)
           << "'";
      d_os << " class='bond-selector bond-" << this_idx << " atom-" << a_idx1
           << " atom-" << a_idx2;
  ...
  for (const auto &at : mol.atoms()) {
    ...
    d_os << "<circle "
         << " cx='" << formatDouble(pos.x) << "'"
         << " cy='" << formatDouble(pos.y) << "'"
         << " r='" << formatDouble(scale() * radius) << "'";
