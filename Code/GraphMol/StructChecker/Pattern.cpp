...
* of atom i+1.
*/
bool RecMatch(const ROMol &mol, std::vector<unsigned> &match,
              unsigned int level, const AugmentedAtom &aa,
              const std::vector<Neighbourhood> &nbp, bool verbose) {
  bool is_new;

  if (level == aa.Ligands.size()) return true;
  ...
  for (unsigned i = 0; i < nbph.Atoms.size(); i++) {
    ...
    if ((atom.getFormalCharge() == au_ligand.Charge ||
         au_ligand.Charge == ANY_CHARGE) &&
    ...
    if ((atom.getFormalCharge() == au_ligand.Charge ||
        ...
        AtomSymbolMatch(atom.getSymbol(), au_ligand.AtomSymbol)) {
      is_new = true;
      while (match.size() <= level + 1) match.push_back(0);
