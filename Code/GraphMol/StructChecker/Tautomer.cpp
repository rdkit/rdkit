...
bool StructCheckTautomer::applyTautomer(unsigned it) {
    ...
    if (toTautomer.getNumAtoms() != fromTautomer.getNumAtoms()) {
        if (Options.Verbose)
            BOOST_LOG(rdInfoLog) << "ERROR: incorrect data toTautomer.getNumAtoms() != fromTautomer.getNumAtoms()\n";
        // incorrect data
        // throw(.....);
        return false;
    }
    ...
    std::vector<unsigned> atomIdxMap(Mol.getNumAtoms()); // matched tau atom indeces

    if( ! SubstructMatch(Mol, *Options.FromTautomer[it], match)) // SSMatch(mp, from_tautomer, SINGLE_MATCH);
        return false;
    if(Options.Verbose)
        BOOST_LOG(rdInfoLog)<<"found match for from_tautomer with "<< nta <<" atoms\n";
    // init
    for (unsigned i = 0; i < Mol.getNumAtoms(); i++)
        atomIdxMap[i] = -1;
    for (MatchVectType::const_iterator mit = match.begin(); mit != match.end(); mit++) {
    ...
    for (RDKit::BondIterator_ bond = Mol.beginBonds(); bond != Mol.endBonds(); bond++) {
