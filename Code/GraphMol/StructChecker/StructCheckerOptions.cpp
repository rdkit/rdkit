    ...
    namespace StructureCheck {
        ...
        bool parseOptionsJSON(const std::string &json, StructCheckerOptions &op) {
            ...
            try {
                ...
                BOOST_LOG(rdErrorLog) << "JSON StructureCheck Options: Unknown exception.\n";
                return false;
            }
            return true;
        }
...
*/
    ...
    while (*str == '(') {
        ...
        else aa.Ligands.back().Charge = 0; //NONE; default
    ...
    if (*str != '\0') { // there are some extra symbols after scan (inside quoted AA string)
...
static 
bool ReadAugmentedAtoms(const std::string &path, std::vector<AugmentedAtom> &atoms) {
/*
    ...
    for (unsigned i = 0; i < n && !feof(fp); i++) {
        ...
        if (fgets(str, sizeof(str), fp)) {
            ...
            if (*s == '"') {
                ...
                {
                    BOOST_LOG(rdErrorLog) << "unsuccessful AA parsing of "<< i+1 <<" "<< str << "\n";
                    fclose(fp);
                    return false;
                }
            }
...
static
bool ReadAAPairs(const std::string &path, std::vector<std::pair<AugmentedAtom, AugmentedAtom> > &trans_pairs) {
/*
* '*.trn' file loader
...
*/
    ...
    if (trans_pairs.size() != n) {
        BOOST_LOG(rdErrorLog) << "syntax error in translation file " << path << " pair " << trans_pairs.size()+1 << "\n";
        return false;
    }
    return true;
}
...
void StructCheckerOptions::setAcidicAugmentedAtoms  (const std::vector<AugmentedAtom> &atoms) {
    AcidicAtoms = atoms;
}
...
void StructCheckerOptions::setGoodAugmentedAtoms  (const std::vector<AugmentedAtom> &atoms) {
    GoodAtoms = atoms;
}

static
bool loadSDF(const std::string &path, std::vector<ROMOL_SPTR> &mols) {
    mols.clear();
    try {
                ...
                if (str.length() >= 4 && 0 == strcmp(str.c_str(), "$RXN")) {
                    while (!in.eof()) {
                        ...
                        if (str.length() >= 4 && 0 == strcmp(str.c_str(), "$MOL")) {
                            ...
                            if (!molFrom) {
                                BOOST_LOG(rdErrorLog) << "RD-file corrupted. "<<path<<" line " << line << "\n"; // log error
                                return false;
                            }
...
void StructCheckerOptions::setTautomerData(const std::vector<ROMOL_SPTR> &from, const std::vector<ROMOL_SPTR> &to) {
    FromTautomer = from;
    ToTautomer = to;
}

static
void parseSMARTS(std::vector<ROMOL_SPTR> &mols, const std::vector<std::string> &smarts) {
    mols.clear();
...
void StructCheckerOptions::parsePatterns(const std::vector<std::string> &smarts) {
    parseSMARTS(Patterns, smarts);
}

void StructCheckerOptions::parseRotatePatterns(const std::vector<std::string> &smarts) {
    parseSMARTS(RotatePatterns, smarts);
}

void StructCheckerOptions::parseStereoPatterns(const std::vector<std::string> &smarts) {
    parseSMARTS(StereoPatterns, smarts);
}

void StructCheckerOptions::parseTautomerData(const std::vector<std::string> &smartsFrom, 
