...
TEST_CASE("CDXML") {
  ...
  SECTION("ChemDraw Template from the synthesis-workshop") {
    ...
    auto mols = MolsFromCDXMLFile(fname);
#ifndef RDKIT_BUILD_CHEMDRAW_SUPPORT
    std::vector<std::string> expected = {
...
#ifndef RDKIT_BUILD_CHEMDRAW_SUPPORT
    // The original Revvity/Cambridgesoft CDX/ML parser doesn't throw an error defaults to singlebond
    {
...
#else
      // The Revvity/Cambridge soft reader has slightly different floating point
      std::vector<std::string> expected = {
