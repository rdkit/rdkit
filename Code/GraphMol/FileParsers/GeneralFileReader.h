...
bool valid(std::string& fileFormat, std::string& compressionFormat){
    ...
    std::vector<std::string> fileFormats{ "sdf", "mae", "smi", "csv", "txt", "tsv", "tdt" };
    ...
    bool flag_fileFormat = std::find(fileFormats.begin(), fileFormats.end(), fileFormat) != fileFormats.end();

    if (!compressionFormat.empty()) {
        bool flag_compressionFormat = std::find(compressionFormats.begin(), compressionFormats.end(), compressionFormat)
        ...
        if (!flag_compressionFormat) {
            ...
            compressionFormat = "";
            flag_fileFormat = std::find(fileFormats.begin(), fileFormats.end(), fileFormat) != fileFormats.end();
        }
    }
...
void determineFormat(const std::string path, std::string& fileFormat, std::string& compressionFormat){
    ...
    if (dots == 0)
        throw std::invalid_argument("Recieved Invalid File Format, no extension or compression");
        ...
        if (!valid(fileFormat, compressionFormat))
            throw std::invalid_argument("Recieved Invalid File or Compression Format");
    }
}


MolSupplier* getSupplier(const std::string& path, const struct SupplierOptions opt){
 		
    std::string fileFormat = "";
		std::string compressionFormat = "";   	
		determineFormat(path, fileFormat, compressionFormat); //! get the file and compression format form the path
