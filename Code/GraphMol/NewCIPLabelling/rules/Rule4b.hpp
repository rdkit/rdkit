...
template <typename A, typename B> class Rule4b : public SequenceRule<A, B>
{
        ...
        if (a->getBeg()->getDigraph()->getCurrRoot() != a->getBeg() ||
            ...
            if (list1.empty() != list2.empty()) {
                throw std::runtime_error(
                    "Ligands should be topologically equivalent!");
            }
            if (list1.size() == 1) {
  ...
  private:
    ...
    Sort<A, B>* getRefSorter(Descriptor refA) const
    {
        ...
        new_rules.push_back(new Rule4b<A, B>(this->getMol(), refA));
        return new Sort<A, B>(new_rules);
    }
};
