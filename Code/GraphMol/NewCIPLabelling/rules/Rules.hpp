...
namespace
{
template <typename Base, typename T> bool is_instance_of(const T* ptr)
{
...
template <typename A, typename B> class Rules : public SequenceRule<A, B>
{
  private:
  ...
  public:
    ...
    Sort<A, B>*
    createSorter(const std::vector<const SequenceRule<A, B>*>& rules)
    {
        auto subrules = std::vector<const SequenceRule<A, B>*>();
        subrules.reserve(rules.size());
        for (const auto& rule : rules) {
            /*
            if (!SORT_BRANCHES_WITH_RULE5 && is_instance_of<Rule5>(rule)) {
              continue;
            }
            */
    ...
    const Sort<A, B> getSorter() const override { return Sort<A, B>(rules); }
