...
template <typename A, typename B> class Node
{
  ...
  public:
    ...
    Node(Digraph<A, B>* g, std::vector<char>&& visit, A atom, short anum,
        ...
        : g{g}, atom{atom}, dist{dist}, atomicnumNum{anum},
          atomicnumDen{aden}, flags{flags}, visit{std::move(visit)}
    {
    ...
    int getAtomicNumNumerator() const { return atomicnumNum; }

    int getAtomicNumDenominator() const { return atomicnumDen; }
    ...
    {
        int new_dist = static_cast<char>(
            ((flags & DUPLICATE) != 0 ? visit[idx] : dist + 1));
        int anum;
        int aden;
        ...
        return new Node(g, std::move(new_visit), atom, anum, aden, new_dist,
                        flags);
    }
