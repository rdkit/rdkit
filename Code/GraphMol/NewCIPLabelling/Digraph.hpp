    ...
    void expand(Node<A, B>* beg)
    {
        ...
        for (const B bond : mol->getBonds(atom)) {
            ...
            if (beg->visit[nbrIdx] == 0) {
                ...
                if (root != beg) {
                    if (mol->getCharge(atom) < 0 &&
                        ...
                        for (int i = 1; i < bord; ++i) {
                            end = beg->newTerminalChild(
                                nbrIdx, nbr, Node<A, B>::BOND_DUPLICATE);
                            ++numNodes;
                            addEdge(beg, bond, end);
                        }
                    }
                ...
                auto end = beg->newTerminalChild(nbrIdx, nbr,
                                                 Node<A, B>::RING_DUPLICATE);
                ++numNodes;
                addEdge(beg, bond, end);
        ...
        for (int i = 0; i < hcnt; ++i) {
            auto end =
                beg->newTerminalChild(-1, nullptr, Node<A, B>::IMPL_HYDROGEN);
            ++numNodes;
