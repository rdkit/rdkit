...
typedef struct {
  ...
  int** cost;
  //int** assignment;  
  int*  my_assig;  //this will be an array of 2*max(num_rows, num_cols)
} hungarian_problem_t;
...
/*free the memory allocated by arrayto matrix */
...
//from an array make a matrix
int** array_to_matrix(int* m, int rows, int cols);

//from an int array make a FLOAT matrix
