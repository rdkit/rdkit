...
void free_array_to_matrix(int **r, int rows) {
	int row=0;
	for(row=0; row<rows; row++) free(r[row]);
	free(r);
}

float** array_to_matrix_float(int* m, int rows, int cols) {
  //This had a memory leak! solved with the code above -> free_array_to_matrix
  int i,j;
  float** r;
  r = (float**)calloc(rows,sizeof(float*));
  for(i=0;i<rows;i++) {
    r[i] = (float*)calloc(cols,sizeof(float));
    for(j=0;j<cols;j++) r[i][j] = (float)m[i*cols+j];
  }
  return r;
}
...
int hungarian_init(hungarian_problem_t* p, int** cost_matrix, int rows, int cols, int mode) {
  ...
  //init the assignment ->this should be OUT
  //p->assignment = (int**)calloc(rows,sizeof(int*));
  //hungarian_test_alloc(p->assignment);

  p->my_assig=(int*)malloc(rows*sizeof(int));
...
void hungarian_solve(hungarian_problem_t* p)
{
  ...
  memset(p->my_assig, 0, p->num_rows*sizeof(int));
