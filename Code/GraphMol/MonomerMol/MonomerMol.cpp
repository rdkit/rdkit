...
std::string MonomerMol::getPolymerId(const ::RDKit::Atom* atom)
{
...
unsigned int MonomerMol::getResidueNumber(const ::RDKit::Atom* atom)
{
    const auto monomer_info = atom->getMonomerInfo();
    if (monomer_info == nullptr) {
        throw std::runtime_error("Atom does not have monomer info");
...
bool isValidChain(const MonomerMol& monomer_mol, std::string_view polymer_id)
{
    ...
    for (size_t i = 1; i < chain.atoms.size(); ++i) {
        ...
        if (MonomerMol::getResidueNumber(bond->getBeginAtom()) >
                MonomerMol::getResidueNumber(bond->getEndAtom()) &&
...
RDKit::Atom* findChainBegin(MonomerMol& monomer_mol)
{
