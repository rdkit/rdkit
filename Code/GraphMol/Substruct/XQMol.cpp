...
std::string ExtendedQueryMol::toBinary() const {
  ...
  std::string pkl;
  if (std::holds_alternative<std::unique_ptr<RWMol>>(xqmol)) {
    streamWrite(ss, ExtendedQueryMolTypes::XQM_MOL);
    MolPickler::pickleMol(*std::get<std::unique_ptr<RWMol>>(xqmol), pkl);
#ifdef RDK_USE_BOOST_SERIALIZATION
  } else if (std::holds_alternative<std::unique_ptr<MolBundle>>(xqmol)) {
    streamWrite(ss, ExtendedQueryMolTypes::XQM_MOLBUNDLE);
    pkl = std::get<std::unique_ptr<MolBundle>>(xqmol)->serialize();
  } else if (std::holds_alternative<std::unique_ptr<TautomerQuery>>(xqmol)) {
    streamWrite(ss, ExtendedQueryMolTypes::XQM_TAUTOMERQUERY);
    pkl = std::get<std::unique_ptr<TautomerQuery>>(xqmol)->serialize();
  } else if (std::holds_alternative<
                 std::vector<std::unique_ptr<TautomerQuery>>>(xqmol)) {
    streamWrite(ss, ExtendedQueryMolTypes::XQM_TAUTOMERBUNDLE);
    const auto &itm =
        std::get<std::vector<std::unique_ptr<TautomerQuery>>>(xqmol);
