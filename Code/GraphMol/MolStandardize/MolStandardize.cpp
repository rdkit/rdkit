...
//	std::cout << "After sanitizeMol: " << MolToSmiles(mol) << std::endl;
	...
	std::cout << "After reionized in cleanup: " << MolToSmiles(*reionized) << std::endl;
...
//
RWMol* fragmentParent(const RWMol &mol, const CleanupParameters &params, bool skip_standardize) {
...
RWMol* chargeParent(const RWMol &mol, const CleanupParameters &params, 
	...
	if (!skip_standardize) {
		...
		std::cout << "After cleanup in chargeparent: " << MolToSmiles(*m) << std::endl;
	}
	std::cout << "After cleanup: " << MolToSmiles(*m) << std::endl;

	RWMOL_SPTR fragparent( fragmentParent(*m, params, true) );
//	delete m;
	std::cout << "After fragmentParent: " << MolToSmiles(*fragparent) << std::endl;
...
RWMol* normalize(const RWMol *mol, const CleanupParameters &params){
	...
	std::shared_ptr<TransformCatalogParams> tparams( new TransformCatalogParams(params.normalizations) );
...
RWMol* reionize(const RWMol *mol, const CleanupParameters &params){
	...
	std::unique_ptr<AcidBaseCatalogParams> abparams( new AcidBaseCatalogParams(params.acidbaseFile) );
...
//	unsigned int ntautomers = tautparams->getNumTautomers();
	...
	std::vector<ROMOL_SPTR> res = te.enumerate(static_cast<ROMol>(*mol), &tautcat);
