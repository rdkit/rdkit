...
RDKitStandardizeMode setMode(const std::string &argv1, const std::string &argv2) {
	RDKitStandardizeMode standardize_mode;
	if (argv1 == "Metal" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::MetalShort;}
	if (argv1 == "Metal" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::MetalLong;}
	if (argv1 == "Fragment" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::FragmentShort;}
	if (argv1 == "Fragment" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::FragmentLong;}
	if (argv1 == "StandardizeSm" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::StandardizeSmShort;}
	if (argv1 == "StandardizeSm" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::StandardizeSmLong;}
	if (argv1 == "Validate" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::ValidateShort;}
	if (argv1 == "Validate" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::ValidateLong;}
	if (argv1 == "Normalize" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::NormalizeShort;}
	if (argv1 == "Normalize" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::NormalizeLong;}
	if (argv1 == "Reionize" && argv2 == "short") {standardize_mode = RDKitStandardizeMode::ReionizeShort;}
	if (argv1 == "Reionize" && argv2 == "long") {standardize_mode = RDKitStandardizeMode::ReionizeLong;}
	// check if argv is within defined enum types
	if (standardize_mode < RDKitStandardizeMode::StandardizeSmShort || standardize_mode > RDKitStandardizeMode::ReionizeLong) {
...
std::vector<std::pair<std::string, std::string>> readCSV(const RDKitStandardizeMode &func) {
	std::string rdbase = std::getenv("RDBASE");
	std::string filename;
	switch (func) {
	case RDKitStandardizeMode::MetalShort:	
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/1kPCS_metals.csv.gz";
		break;
	case RDKitStandardizeMode::MetalLong:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/100kPCS_metals.csv.gz";
		break;
	case RDKitStandardizeMode::StandardizeSmShort:	
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/1kPCS_standardize_sm.csv.gz";
		break;
	case RDKitStandardizeMode::StandardizeSmLong:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/100kPCS_standardize_sm.csv.gz";
		break;
	...
	case RDKitStandardizeMode::FragmentLong:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/100kPCS_fragment.csv.gz";
		break;
	case RDKitStandardizeMode::ReionizeShort:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/1kPCS_reionize.csv.gz";
		break;
	case RDKitStandardizeMode::ReionizeLong:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/100kPCS_reionize.csv.gz";
		break;
	case RDKitStandardizeMode::NormalizeShort:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/1kPCS_normalize.csv.gz";
		break;
	case RDKitStandardizeMode::NormalizeLong:
		filename = rdbase + "/rdkit/Chem/MolStandardize/test_data/100kPCS_normalize.csv.gz";
		break;
	}
...
int main(int argc, char* argv[]){
	...
	std::vector<std::pair<std::string, std::string>> res = readCSV(standardize_mode);
	if (argc < 3) {
