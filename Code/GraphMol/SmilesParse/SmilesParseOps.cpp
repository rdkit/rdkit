// $Id: SmilesParseOps.cpp 4974 2006-02-18 00:49:21Z glandrum $
//
...
namespace SmilesParseOps{
  ...
  void AddFragToMol(RWMol *mol,RWMol *frag,Bond::BondType bondOrder,
	...
	for(INT_VECT::iterator ivIt=tmpVect.begin();ivIt!=tmpVect.end();ivIt++){
	  // if the ring closure is not already a bond, don't touch it:
	  if(*ivIt>=0) (*ivIt) += nOrigBonds;
	}
  ...
  void CloseMolRings(RWMol *mol,bool toleratePartials){
	...
	for(aplci=bookmarkedAtomsToRemove.begin();aplci!=bookmarkedAtomsToRemove.end();aplci++){
	  mol->clearAtomBookmark(mark,*aplci);
	}
      } else {
	bookmarkIt++;
      }
