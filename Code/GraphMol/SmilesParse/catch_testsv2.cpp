...
TEST_CASE("Github #1972", "[SMILES][bug]") {
  SECTION("basics") {
    ...
    for (const auto &pr : smiles) {
      std::unique_ptr<ROMol> m1(SmilesParser::SmilesToMol(pr[0]));
      std::unique_ptr<ROMol> m2(SmilesParser::SmilesToMol(pr[1]));
      REQUIRE(m1);
      REQUIRE(m2);
...
TEST_CASE("Github #2029", "[SMILES][bug]") {
  SECTION("wedging") {
    std::unique_ptr<ROMol> m1(SmilesParser::SmilesToMol("CN[C@H](Cl)C(=O)O"));
    REQUIRE(m1);
  ...
  SECTION("direction") {
    std::unique_ptr<ROMol> m1(SmilesParser::SmilesToMol("C/C=C/C"));
    REQUIRE(m1);
    bool doKekule = false, allBondsExplicit = false;
  ...
  SECTION("aromatic double bonds") {
    std::unique_ptr<RWMol> m1(SmilesParser::SmilesToMol("c1ccccc1"));
    REQUIRE(m1);
...
TEST_CASE(
  ...
  SECTION("#2197") {
    for (const auto &sma : smarts) {
      std::unique_ptr<ROMol> mol(SmilesParser::SmartsToMol(sma));
      REQUIRE(mol);
      CHECK(6 == mol->getNumAtoms());
...
TEST_CASE("Github #2148", "[bug][Smiles][Smarts]") {
  ...
  SECTION("SMILES edges") {
...

    {
      ...
      for (auto smi : smis) {
        std::unique_ptr<RWMol> mol(SmilesParser::SmilesToMol(smi));
        REQUIRE(mol);
...
TEST_CASE(
  ...
  SECTION("original_report") {
    ...
    CHECK(smarts == "*!@CCC");
    std::unique_ptr<RWMol> q2(SmilesParser::SmartsToMol(smarts));
    REQUIRE(q2);
  }
...
TEST_CASE("d primitive in SMARTS", "[smarts][extension]") {
  ...
  ps.removeHs = false;
  std::unique_ptr<ROMol> m(SmilesParser::SmilesToMol("[H]OCO[2H]", ps));
  REQUIRE(m);
...
TEST_CASE("Github #2788: doKekule=true should kekulize the molecule",
  ...
  SECTION("can thrown exceptions") {
    SmilesParserParams ps;
    ps.sanitize = false;
    std::unique_ptr<RWMol> m{SmilesParser::SmilesToMol("c1ccnc1", ps)};
    REQUIRE(m);
    bool doIsomeric = true;
    bool doKekule = false;
    {
...
TEST_CASE("bogus recursive SMARTS", "[smarts]") {
  ...
  CHECK(SmilesParser::SmartsToMol(sma) == nullptr);
}
...
TEST_CASE("Github #4319 add CXSMARTS support") {
  ...
  SECTION("params") {
    ...
    const std::unique_ptr<RWMol> mol(SmilesParser::SmartsToMol(sma, ps));
    REQUIRE(mol);
    REQUIRE(mol->getNumAtoms() == 3);
    CHECK(mol->getAtomWithIdx(0)->getProp<std::string>(
              common_properties::atomLabel) == "foo");
