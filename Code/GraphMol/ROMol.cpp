...
Atom *ROMol::getAtomWithIdx(unsigned int idx) {
  ...
  URANGE_CHECK(idx + 1, getNumAtoms());
...
Bond *ROMol::getBondWithIdx(unsigned int idx) {
  ...
  URANGE_CHECK(idx + 1, getNumBonds());
...
Bond *ROMol::getBondBetweenAtoms(unsigned int idx1, unsigned int idx2) {
  URANGE_CHECK(idx1 + 1, getNumAtoms());
  URANGE_CHECK(idx2 + 1, getNumAtoms());
  Bond *res = 0;
...
unsigned int ROMol::addBond(Bond *bond_pin, bool takeOwnership) {
  ...
  URANGE_CHECK(bond_pin->getBeginAtomIdx() + 1, getNumAtoms());
  URANGE_CHECK(bond_pin->getEndAtomIdx() + 1, getNumAtoms());
