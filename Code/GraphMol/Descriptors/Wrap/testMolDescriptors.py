...
class TestCase(unittest.TestCase):
  ...
  def testBCUT(self):
    smiles = ("c1ccccc1S", "c1cscc1", "CC(=S)C", "CSC", "CS(=O)C", "CP(C)C", "CP=O", "CP(C)(C)=O",
              "C[PH](C)=O")
    expected = {rdMD.BCUTOptions.PERLMAN_MATRIX:
      [
        [
      ...
      rdMD.BCUTOptions.BURDEN_MATRIX:
        [
          [32.067603779466225, 11.711902531746654, 0.26826123011407327, -0.35493862681264965,
           0.6795094498336206, -0.13835188602786477, 7.593469013339848, 2.6073896043130262],
          [32.069244033919745, 11.768912935676253, 0.25240484784547407, -0.3237686635021173,
           0.7143170154236707, -0.08398706432373429, 6.704456597371263, 3.1079129356762647],
          [32.06919906383734, 11.854852088793868, 0.22313369726482019, -0.30545308190133347,
           0.6957958261969219, -0.36860354106258836, 7.6079933899309315, 2.4943423287748283],
          [32.06820660902989, 12.009999999999998, 0.07968701264657263, -0.266226912059352,
           0.675754650561241, -0.23005465056124114, 7.595997936867289, 2.7490020631327146],
          [32.070950641470546, 12.01, 0.22244670084533194, -0.38820049492327197,
           0.15574525284115495, -0.4560202289122022, 7.376920119972417, 0.7707060703746403],
          [30.975914262728654, 12.009999999999998, 0.11438317827354712, -0.27367058167322095,
           0.8943257058545578, -0.2346257058545567, 6.928697324500968, 2.7463026754990336],
          [30.97758233067376, 12.010361924843584, 0.2777248909105451, -0.36586767626297073,
           0.9147299920357694, -0.22136705285628655, 6.9299620030571285, 0.679429814931328],
          [30.97885840652821, 12.009999999999993, 0.27887573460811294, -0.4284350012177264,
           0.9350979639500913, -0.24751182966883922, 6.935744822820914, 0.6794227671947708],
          [30.978220356424462, 12.00999999999999, 0.24299236455882867, -0.4272978698025007,
           0.9249919865673539, -0.2351548874666096, 6.932854719019769, 0.6794262926494323]
        ]
    }
    
    for opt in (rdMD.BCUTOptions.PERLMAN_MATRIX,
                rdMD.BCUTOptions.BURDEN_MATRIX,):
      for i, smi in enumerate(smiles):
        mol = Chem.MolFromSmiles(smi)
        res = rdMD.BCUT2D(mol, opt)
        self.assertEqual(len(res), 8)
        for rv, ev in zip(res, expected[opt][i]):
