...
class streambuf : public std::basic_streambuf<char> {
  ...
  */
   ...
   */
  streambuf(nb::object &python_file_obj, std::size_t buffer_size_ = 0)
      : py_read(nb::getattr(python_file_obj, "read", nb::none())),
        py_write(nb::getattr(python_file_obj, "write", nb::none())),
        py_seek(nb::getattr(python_file_obj, "seek", nb::none())),
        py_tell(nb::getattr(python_file_obj, "tell", nb::none())),
    ...
    if (!py_tell.is_none()) {
      try {
        off_type py_pos = nb::cast<off_type>(py_tell());
        if (!py_seek.is_none()) {
  ...
  streambuf(nb::object &python_file_obj, char mode,
            std::size_t buffer_size_ = 0)
      : streambuf(python_file_obj, buffer_size_) {
#if 1
    auto io_mod = nb::module_::import_("io");
    CHECK_INVARIANT(io_mod, "module not found");
    nb::object iobase = nb::getattr(io_mod, "TextIOBase");
    ...
    // the future
    static nb::object io_mod = nb::object();
    static nb::object iobase = nb::object();
    if (!io_mod) io_mod = nb::import("io");
    if (io_mod && !iobase) iobase = nb::getattr(io_mod, "TextIOBase");
    CHECK_INVARIANT(io_mod, "module not found");
    CHECK_INVARIANT(iobase, "base class not found");
#endif
  ...
  pos_type seekoff(off_type off, std::ios_base::seekdir way,
    ...
    */
    ...
    if (!result) {
      ...
      py_seek(off, whence);
      result = off_type(nb::cast<off_type>(py_tell()));
      if (which == std::ios_base::in) {
        underflow();
      }
    }
...
struct streambuf_capsule {
  ...
  streambuf_capsule(nb::object &python_file_obj, std::size_t buffer_size = 0)
      : python_streambuf(python_file_obj, buffer_size) {}
};

struct ostream : private streambuf_capsule, streambuf::ostream {
  ostream(nb::object &python_file_obj, std::size_t buffer_size = 0)
