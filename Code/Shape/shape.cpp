...
SolutionInfo shape(RDKit::ROMol& refMol, RDKit::ROMol& dbMol, unsigned int nBestHits, bool scoreOnly, unsigned int maxIter, std::string whichScore) {
        ...
        if (bestHits != 0) { bestHits = new BestResults(nBestHits); }    
        ...
        if ( !scoreOnly ) { initOrientation(refVolume); }
        ...
        if (scoreOnly)
        {
            ...
            res.rotor[0] = 1.0;
            bestScore = getScore(whichScore, res.overlap, refVolume.overlap, dbVolume.overlap);
        }
        else
        {
            ...
            for (unsigned int l(0); l < 4; ++l)
            {
                ...
                double ss = getScore(whichScore, nextRes.overlap, refVolume.overlap, dbVolume.overlap);
                if (ss > bestScore)
                {
                    res = nextRes;
                    bestScore = ss;
