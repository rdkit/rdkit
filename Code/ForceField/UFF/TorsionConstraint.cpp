...
TorsionConstraintContrib::TorsionConstraintContrib(
  ...
  URANGE_CHECK(idx1 + 1, owner->positions().size());
  URANGE_CHECK(idx2 + 1, owner->positions().size());
  URANGE_CHECK(idx3 + 1, owner->positions().size());
  URANGE_CHECK(idx4 + 1, owner->positions().size());
...
TorsionConstraintContrib::TorsionConstraintContrib(
  ...
  URANGE_CHECK(idx1 + 1, pos.size());
  URANGE_CHECK(idx2 + 1, pos.size());
  URANGE_CHECK(idx3 + 1, pos.size());
  URANGE_CHECK(idx4 + 1, pos.size());
  PRECONDITION((!(maxDihedralDeg < minDihedralDeg)) &&
                   ((maxDihedralDeg - minDihedralDeg) < 360.0),
               "bad bounds");
